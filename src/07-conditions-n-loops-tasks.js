...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const result = `${isStartIncluded ? '[' : '('}${a < b ? a : b}, ${b > a ? b : a}${isEndIncluded ? ']' : ')'}`;
  return result;
}
...
function getDigitalRoot(num) {
  ...
  if (sum1 > 9) {
    const arr2 = sum1.toString().split('').map((i) => Number(i));
    let sum2 = 0;
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openingBrackets.includes(char)) {
      ...
      if (!lastOpeningBracket
        || openingBrackets.indexOf(lastOpeningBracket) !== closingBrackets.indexOf(char)
      ) {
        return false;
      }
    }
...
function evaluateTicTacToePosition(/* position */) {
  ...
  //   }
  // }
  // return position.flat().every((cell) => cell) ? 'draw' : undefined;
}
